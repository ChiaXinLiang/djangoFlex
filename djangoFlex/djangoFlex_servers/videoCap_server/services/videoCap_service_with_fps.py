...
class VideoCapService:
    def __init__(self):
        ...
        self.frame_interval = 5  # Set the frame interval in seconds
        self._load_configs()
    ...
    def stop_server(self, rtmp_url):
        ...
        with transaction.atomic():
            ...
            config.save()
            self.cleanup_old_frames(rtmp_url)
    ...
    def _capture_loop(self, rtmp_url):
        ...
        while self.running[rtmp_url]:
            if rtmp_url in self.caps and self.caps[rtmp_url] is not None and self.caps[rtmp_url].isOpened():
                if self._process_frame(rtmp_url, last_save_time):
                    reconnect_start_time = None
                    reconnect_attempts = 0
    ...
    def _process_frame(self, rtmp_url, last_save_time):
        ...
        try:
            ...
            current_time = time.time()
            if current_time - last_save_time >= self.frame_interval:
                asyncio.run(self.update_frame(rtmp_url, frame))
    ...
    async def update_frame(self, rtmp_url, frame):
        ...
        loop.run_in_executor(self.executor, self._save_frame_to_db, rtmp_url, frame_bytes)

    def _save_frame_to_db(self, rtmp_url, frame_bytes):
        with transaction.atomic():
            config = self.configs[rtmp_url]
            CurrentFrame.objects.create(
                config=config,
                frame_data=frame_bytes,
                timestamp=timezone.now()
            )

    def cleanup_old_frames(self, rtmp_url, reference_timestamp=None):
        config = self.configs[rtmp_url]
        if reference_timestamp is None:
            reference_timestamp = timezone.now()
        CurrentFrame.objects.filter(config=config, timestamp__lt=reference_timestamp).delete()
