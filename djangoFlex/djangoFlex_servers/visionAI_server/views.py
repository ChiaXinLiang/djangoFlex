...
class ViolationDetectView(APIView):
    ...
    def post(self, request):
        ...
        if action == 'detect':
            ...
            is_running = violation_detect_service.is_running
            return Response({"message": message, "is_running": is_running}, status=status.HTTP_200_OK)
        elif action == 'stop':
            violation_detect_service.stop_service()
            return Response({"message": "Violation detection service stopped", "is_running": False}, status=status.HTTP_200_OK)
        elif action == 'status':
            is_running = violation_detect_service.is_running
